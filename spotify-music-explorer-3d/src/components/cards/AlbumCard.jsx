...
export default function AlbumCard({ album, idx = 0 }) {
  const navigate = useNavigate()
  const { isFavorite, addFavorite, removeFavorite } = useFavorites()
  const [hov, setHov] = useState(false)
  const fav = isFavorite(album.id)
  const imgSrc = album.imageUrl || album.images?.[0]?.url
    || `https://picsum.photos/seed/${encodeURIComponent(album.name)}album/300/300`
  ...
  function toggleFav(e) {
    ...
    if (fav) {
      ...
      addFavorite({ ...album, itemType: 'album' })
  ...
  return (
    ...
    >
      <div style={{ position: 'relative', aspectRatio: '1', overflow: 'hidden', background: 'var(--bg-elevated)' }}>
        <img src={imgSrc} alt={album.name} style={{
      ...
      <div style={{ padding: '11px 13px 13px' }}>
        ...
        <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginTop: 3 }}>
          {album.release_date?.slice(0, 4)}{album.total_tracks ? ` · ${album.total_tracks} canciones` : ''}
        </div>
      </div>
